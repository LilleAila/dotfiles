{ self, lib, ... }:
{
  configurations.nixos.x220-nix.module =
    {
      config,
      pkgs,
      ...
    }:
    {
      ...
      hm = {
        ...
        wayland.windowManager.hyprland.settings.input.kb_options =
          lib.mkForce "ctrl:nocaps,shift:both_capslock_cancel,altwin:menu_win";

        sops.secrets."ssh/x220".path = "${config.hm.home.homeDirectory}/.ssh/id_ed25519";
        home.file.".ssh/id_ed25519.pub".text = self.keys.ssh.x220.public;

        sops.secrets."yubikey/u2f_keys".path = "${config.hm.home.homeDirectory}/.config/Yubico/u2f_keys";

        sops.secrets."syncthing/x220/cert".path =
          "${config.hm.home.homeDirectory}/.config/syncthing/cert.pem";
        sops.secrets."syncthing/x220/key".path =
