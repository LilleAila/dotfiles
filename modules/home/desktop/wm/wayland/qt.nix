    ...
    }:
    {
      ...
      config = lib.mkIf config.settings.qt.enable (
        let
          ...
          baseConfig = {
            Appearance = {
              color_scheme_path = "${config.home.homeDirectory}/.config/qt5ct/colors/${self.colorScheme.slug}.conf";
              custom_palette = true;
        ...
        in
        {
          ...
          home.file.".config/qt6ct/colors/${self.colorScheme.slug}.conf".text = colorScheme;
